...
def yayanxd():
    os.system('clear')
    r=requests.Session()
    print(f'{O} *{N} TYPE OPEN TO BUY COOKIE')
    cookie = input("\n %s[%s?%s] Cookies : %s"% (N,K,N,O))
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,H));time.sleep(3);os.system('xdg-open https://wa.me/+2348110044418');yayanxd()
    try:
        ...
        print(' %s*%s mohon untuk menggunakan sc ini sewajarnya, PLEASE USE THIS TOOL WISELY .WE ARE NOT RESPONSIBLE IF MISUSED ONWER KARMA DAVID...'%(O,N));time.sleep(2)
...
def moch_yayan():
    ...
    print(' [%s01%s]. Crack id from Group'%(U,P));time.sleep(0.03)
    print(' [%s02%s]. Crack id from public'%(O,N));time.sleep(0.03)
    print(' [%s03%s]. Crack id from followers'%(U,P));time.sleep(0.03)
    print(' [%s04%s]. Crack id from like'%(O,N));time.sleep(0.03)
    print(' [%s05%s]. Crack id from random'%(U,P));time.sleep(0.03)
    print(' [%s06%s]. Crack id from post'%(O,N));time.sleep(0.03)
    print(' [%s07%s]. Auto Checkpoint Detector'%(O,N));time.sleep(0.03)
    print(' [%s08%s]. User agent settings'%(U,P));time.sleep(0.03)
    print(' [%s09%s]. check crack result'%(O,N));time.sleep(0.03)
    print(' [%s10%s]. Upgrade to%s premium%s'%(U,P,U,P));time.sleep(0.03)
    print(' [%s00%s]. logout (%shapus cookie%s)'%(O,N,O,N));time.sleep(0.03)
    pepek = input('\n [%s*%s] menu : '%(H,N))
    if pepek == '':
...
def cok():
    ...
    try:
        ...
        exit("[%s!%s] rerun the command by typing python dharma.py"%(M,N))
    except KeyError:
...
class __crack__:
    ...
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            print('\n [ pilih method login - silahkan coba satu² ]\n')
            print(' [%s1%s]. method API (fast)'%(U,P))
            print(' [%s2%s]. method mbasic (slow)'%(O,N))
            print(' [%s3%s]. method mobile (Fast) [\033[92mRecommended\033[0m]'%(U,P))
            self.__pler__()
        else:
...
#    <- Bot followers ->
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print('\n [%s+%s] Karma OK disimpan ke -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta))
            print(' [%s+%s] Karma CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta))
            print('\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N))
